...
public class GoldenRubberDuckBlock extends RubberDuckBlock {
    ...
    public InteractionResult use(@Nonnull BlockState state, @Nonnull Level level, @Nonnull BlockPos pos, 
        ...
        if (hand == InteractionHand.MAIN_HAND) {
            if (!level.isClientSide) {
                // Play golden duck quack sound
                level.playSound(null, pos, AllSounds.GOLDEN_RUBBER_DUCK_QUACK.get(), SoundSource.BLOCKS, 1.5F, 
                               1.0F + (level.getRandom().nextFloat() * 0.1F));
                
                // Give Hero of the Village effect for 10 seconds
    ...
    public void onRemove(BlockState state, Level level, BlockPos pos, BlockState newState, boolean isMoving) {
        if (!state.is(newState.getBlock())) {
            if (!level.isClientSide) {
                level.playSound(null, pos, AllSounds.GOLDEN_RUBBER_DUCK_BLOCK_EVENT.get(), SoundSource.BLOCKS, 1.5F, 
                              1.0F + (level.getRandom().nextFloat() * 0.1F));
            }
        }
